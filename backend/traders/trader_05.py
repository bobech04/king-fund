...
class Trader(BaseTrader):
    """Groupe A — EU Valeurs Sous-suivies · TTE.PA momentum agressif + FMP."""

    def __init__(self, trader_id: int, starting_capital: float):
        super().__init__(trader_id, starting_capital)
        self.name     = "BOLT"
        self.strategy = "Momentum agressif · TTE.PA + FMP + Liquidité"
        self._symbol  = "TTE.PA"
    ...
    def decide(self, prices: dict) -> dict:
        ...
        if sig == "buy":
            fraction = 0.85 * max(0.3, 1.0 + (fund * 0.3 + liq * 0.2))
            return self._buy(self._symbol, min(1.0, fraction), prices)
        if sig == "sell":
            fraction = min(1.0, 0.75 + max(0.0, -liq) * 0.25)
