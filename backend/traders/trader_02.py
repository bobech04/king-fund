...
class Trader(BaseTrader):
    """Groupe A — EU Valeurs Sous-suivies · GTT.PA momentum modéré + FMP."""

    def __init__(self, trader_id: int, starting_capital: float):
        super().__init__(trader_id, starting_capital)
        self.name     = "SURGE"
        self.strategy = "Momentum modéré · GTT.PA + FMP + Liquidité"
        self._symbol  = "GTT.PA"
    ...
    def decide(self, prices: dict) -> dict:
        ...
        if sig == "buy" and fund > -0.30:
            fraction = 0.7 * max(0.3, 1.0 + fund * 0.3)
